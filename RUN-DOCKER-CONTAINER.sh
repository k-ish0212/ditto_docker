...
if [ -z $DITTO_PROJECT_NAME ]; then
  PROJECT=$DITTO_PROJECT_NAME
  echo "Set DITTO_PROJECT_NAME (e.g. 'export DITTO_PROJECT_NAME=mytest')"
  exit 1
fi
PROJECT=$DITTO_PROJECT_NAME
CONTAINER="${PROJECT}_ditto_1"
...
case "$2" in
  ...
  ;;
  ( "moveit")
  docker exec -i -t ${CONTAINER} bash -i -c "source ~/scripts/run-roslaunch-repeatedly.sh"
  ;;
  ( * )
